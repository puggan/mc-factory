...
public class FactoryScreen extends ContainerScreen<FactoryContainer> {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        recipeBookGui.render(matrixStack, mouseX, mouseY, partialTicks);
        ...
        for (int slotIndex = FactoryEntity.resultSlotIndex + 1; slotIndex < FactoryEntity.outputSlotIndex; ++slotIndex) {
            ...
            if (slot instanceof ItemSlot) {
                ...
                AbstractGui.fill(matrixStack, slot.xPos, slot.yPos, slot.xPos + 15, slot.yPos + 15, color);
            }
        }
        if (ghostItems) {
            RenderSystem.depthFunc(515);
        ...
        // recipeBookGui.renderGhostRecipe(guiLeft, guiTop, true, partialTicks); #MCP TODO
        recipeBookGui.func_230477_a_(matrixStack, guiLeft, guiTop, true, partialTicks);
        renderHoveredTooltip(matrixStack, mouseX, mouseY);
        // recipeBookGui.renderTooltip(guiLeft, guiTop, mouseX, mouseY); #MCP TODO
        recipeBookGui.func_238924_c_(matrixStack, guiLeft, guiTop, mouseX, mouseY);

        this.setFocusedDefault(recipeBookGui);
    }
    ...
    protected void drawGuiContainerBackgroundLayer(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY) {
        ...
        blit(matrixStack, guiLeft, guiTop, 0, 0, 0, xSize, ySize, 256, 256);
        renderSlotsBackgrounds(matrixStack);
    }

    private void renderSlotsBackgrounds(MatrixStack matrixStack) {
        ...
        for (Slot slot : disabledSlots) {
            // slot off: 151, 16, 18, 18
            blit(matrixStack, slot.xPos, slot.yPos, 151, 16, 18, 18);
        }
        for (Slot slot : normalSlots) {
            // slot on: 152, 83, 18, 18
            blit(matrixStack, slot.xPos, slot.yPos, 152, 83, 18, 18);
        }
        RenderSystem.popMatrix();
    }
}
